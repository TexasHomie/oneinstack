...
Install_OpenResty() {
  ...
  ./configure --prefix=${openresty_install_dir} --user=${run_user} --group=${run_group} --with-http_sub_module --with-http_stub_status_module --with-file-aio --with-threads --with-stream --with-http_geoip_module --with-http_random_index_module --with-http_secure_link_module --with-stream_ssl_module --with-poll_module --with-http_v2_module --with-http_ssl_module --with-http_gzip_static_module --with-http_realip_module --with-http_flv_module --with-http_mp4_module --with-openssl=../openssl-${openssl11_ver} --with-pcre=../pcre-${pcre_ver} --with-pcre-jit --with-cpu-opt=CPU --with-zlib-asm=CPU --with-cc-opt='-O3' --with-ld-opt='-ljemalloc -Wl,-u,pcre_version' ${nginx_modules_options}
